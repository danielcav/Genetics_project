...
print("No, because the distributions of densitites for both genders are almost identical (same for the boxplots), they look very similar to a normal distribution centered around 21. If gender was a covariate, we would have seen different density distributions meaning that having a certain gender boosts the chances of having a certain cholesterol level, which is not the case here.")
...
print("No, we don't need to correct the population structure because the data correctly represents the 3 different groups of people.")
...
summary_p_beta = data.gwas
rownames(summary_p_beta) = c("Beta", "p-value") 
colnames(summary_p_beta) = c(1:11382)
#P and Beta values are available in the summary_p_beta table in the environment section
#Rank of SNPs.
